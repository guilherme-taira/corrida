        ...
        <v-card>
            ...
            <v-card-text>
                <v-form @submit.prevent="salvarEdicao" ref="form">
                    <v-text-field label="Título do Evento" v-model="form.name" required />
                    <v-text-field label="Local do Evento" v-model="form.local" />
                    <v-text-field label="Cidade do Evento" v-model="form.cidade" />

                    <v-switch label="Exibir Tempo Líquido" v-model="form.exibir_tempo_liquido" />
                    <v-switch label="Exibir GAP" v-model="form.exibir_gap" />
                    <v-switch label="Exibir Tempo Bruto" v-model="form.exibir_tempo_bruto" />

                    <!-- Mostrar imagem atual -->
                    <div v-if="form.imagemAtual" class="mt-3">
                        <strong>Imagem Atual:</strong>
                        <v-img :src="`/storage/${form.imagemAtual}`" max-width="200" class="my-2" />
                    </div>

                    <v-file-input label="Nova Imagem do Evento" accept="image/*" v-model="form.imagem" show-size />

                    <!-- Mostrar banner atual -->
                    <div v-if="form.bannerAtual" class="mt-3">
                        <strong>Banner Atual:</strong>
                        <v-img :src="`/storage/${form.bannerAtual}`" max-width="400" class="my-2" />
                    </div>

                    <v-file-input label="Novo Banner do Evento" accept="image/*" v-model="form.banner" show-size />

                    <!-- Mostrar certificado atual -->
                    <div v-if="form.certificadoAtual" class="mt-3">
                        <strong>Certificado Atual:</strong>
                        <v-img :src="`/storage/${form.certificadoAtual}`" max-width="400" class="my-2" />
                    </div>

                    <v-file-input label="Novo Certificado do Evento" accept="image/*" v-model="form.certificado" show-size />
...
function salvarEdicao() {
  ...
  data.append("exibir_tempo_liquido", form.value.exibir_tempo_liquido ? 1 : 0);
  ...
  if (form.value.certificado) data.append("certificado", form.value.certificado);
